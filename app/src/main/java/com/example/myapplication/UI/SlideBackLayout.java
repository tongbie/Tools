...
public class SlideBackLayout extends FrameLayout {
    private ViewGroup mDecorView;//当前Activity的DecorView
    private View mRootView;//DecorView下的LinearLayout
    private Activity mActivity;//需要边缘滑动删除的Activity
    private ViewDragHelper mViewDragHelper;//Drag助手类
    private float mSlideWidth;//触发退出当前Activity的宽度
    private int mScreenWidth;//屏幕宽度
    private int mScreenHeight;//屏幕高度
    private Paint mPaint;//画笔，用来绘制阴影效果
    ...
    private void init(Context context) {
        /*必须是传入Activity*/
        mActivity = (Activity) context;//构造ViewDragHelper
        mViewDragHelper = ViewDragHelper.create(this, new DragCallback());//设置从左边缘捕捉View
        mViewDragHelper.setEdgeTrackingEnabled(ViewDragHelper.EDGE_LEFT);//初始化画笔
        mPaint = new Paint();
    ...
    class DragCallback extends ViewDragHelper.Callback {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (left <= mSlideWidth) {//缓慢滑动的方法,小于触发条件，滚回去
